...
import { FiTrash2, FiUser, FiFileText, FiCheckCircle, FiUpload, FiX, FiRefreshCw, FiAlertTriangle } from 'react-icons/fi';
import { getImageUrl } from '@/Utils/url';

/**
 * EmployeeDelete renders the official deletion authorization form for operators.
 * Single Responsibility: Presenting the deletion authorization gate and submitting deletion request.
 */
export default function EmployeeDelete({ employee }) {
    ...
    return (
        <OperatorLayout>
            <PageHeader
                title="Hapus Pegawai"
                subtitle="Otorisasi dan pengajuan penghapusan data pegawai ke Dinas."
            />
            ...
            <div className="w-full max-w-2xl mx-auto px-4 sm:px-0">
                <GlassCard className="rounded-3xl border border-zinc-200/80 shadow-xs bg-white/90">
                    <div className="text-center pb-6 border-b border-zinc-100">
                        ...
                        <h3 className="text-xl font-bold tracking-tight text-zinc-900">Otorisasi Penghapusan Diperlukan</h3>
                        <p className="text-sm text-zinc-500 mt-2 max-w-md mx-auto leading-relaxed">
                            Untuk menghapus data pegawai <span className="font-semibold text-zinc-900">{employee.name}</span>,
                    ...
                    {/* Notice */}
                    <div className="mt-6 p-4 bg-zinc-50 rounded-2xl border border-zinc-200/80 flex items-start gap-3">
                        <FiAlertTriangle className="w-5 h-5 text-zinc-700 shrink-0 mt-0.5" />
                        <div className="text-xs text-zinc-600 leading-relaxed">
                            <span className="font-semibold text-zinc-900">Ketentuan Penghapusan:</span> Setelah pengajuan disetujui oleh Dinas, data pegawai akan dipindahkan ke <strong className="text-zinc-900">Arsip Pegawai</strong> dan tidak hilang permanen dari sistem.
                        </div>
                    </div>
                    ...
                    {/* Upload Surat Perintah & Alasan Form */}
                    <form onSubmit={handleSubmit} className="space-y-5">
                        <div className="space-y-2">
                            ...
                            >
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-1">
                                        <div className="flex items-center gap-3 text-left min-w-0">
                                            ...
                                            <div className="truncate">
                                                <p className="font-semibold text-sm text-zinc-900 truncate" title={data.surat_perintah.name}>
                                                    {data.surat_perintah.name}
                                                </p>
                                                <p className="text-xs text-zinc-500 font-mono mt-0.5">
                                                    {formatFileSize(data.surat_perintah.size)} &bull; Siap diverifikasi
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <input
                                    ...
                                    className={data.surat_perintah ? 'hidden' : 'absolute inset-0 w-full h-full opacity-0 cursor-pointer'}
                                />
                            </div>
                            {errors.surat_perintah && (
                                <p className="text-rose-600 text-xs mt-1">{errors.surat_perintah}</p>
                            )}
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-5 border-t border-zinc-100">
                            ...
                            <button
                                type="submit"
                                disabled={processing || !data.surat_perintah || !data.reason.trim()}
                            ...
                            >
                                ...
                                <span>{processing ? 'Mengirim Pengajuan...' : 'Kirim Pengajuan Penghapusan'}</span>
                            </button>
                        </div>
                    </form>
                </GlassCard>
